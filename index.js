...
async function fetchData() {
  ...
  const response = await fetch(
    "https://v2jpvqk9l2.execute-api.us-east-1.amazonaws.com/Dev/test",
    {
      method: "GET",
      headers: {
...
async function addTransaction() {
  ...
  el.innerHTML = `<div id="loader" class="loader" style="display: none;"></div>`;
...
async function render() {
  ...
  if (!list.length) {
    el.innerHTML = `<div class="empty"><div class="empty-icon">💸</div><div>No transactions yet</div></div>`;
    return;
  }
  el.innerHTML = list
    .map((t) => {
...
async function renderCatChart() {
  ...
  if (!sorted.length) {
    el.innerHTML =
      '<div style="color:var(--text3);font-size:13px">No expense data yet</div>';
    return;
  }
  el.innerHTML = sorted
    .map(([cat, val]) => {
...
async function renderBudget() {
  ...
  document.getElementById("budget-spent").textContent =
    fmt(currentMonthExpenses);
  document.getElementById("budget-set").textContent = budget
    ? fmt(budget)
    : "Not set";
  if (budget) {
    const pct = Math.min(
    ...
    bar.style.width = pct + "%";
    bar.className =
      "big-bar-fill" + (pct >= 100 ? " danger" : pct >= 75 ? " warn" : "");
...
document.getElementById("inp-date").value = new Date()
  .toISOString()
  .split("T")[0];
document.getElementById("month-label").textContent = new Date().toLocaleString(
  "en-IN",
